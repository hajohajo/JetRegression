...
Global_variables = list(set(list(test_data.filter(regex='jet')))-set(Ccand_variables+Ncand_variables+Pcand_variables))
...
test_data['Response_DNN'] = test_data['DNN_corr_jet_pT']/test_data['genJetPt']
...
bin_dict = {'QG_ptD':np.arange(0.1, 1.05, 0.05),
            'QG_axis2':np.arange(0.0, 0.15, 0.005),
            'QG_mult':np.arange(0.0, 81.0, 1.0),
            'genJetPt':np.arange(0.0, 1020.0, 20.0)}
...
for variables in to_plot:
    ...
    for i in range(1, len(binning)):
        ...
        errors_l1l2l3[i-1] = sem(test_data[variables[1]][indices == i])
        errors_DNN[i-1] = sem(test_data[variables[1]][indices == i])
    ...
    plt.scatter(x, means_l1l2l3, label='L1L2L3', color='blue')
    plt.fill_between(x, means_l1l2l3-errors_l1l2l3, means_l1l2l3+errors_l1l2l3,
                     alpha=0.4, label='$\pm 1\sigma$', color='green')
    plt.scatter(x, means_DNN, label='DNN', color='orange')
